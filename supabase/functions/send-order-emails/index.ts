...
Deno.serve(async (req) => {
  ...
  try {
    const { customerEmail, customerName, orderItems, orderId, totalAmount } = await req.json();
    ...
    const productList = orderItems
      .map((item: any) => `• ${item.product_name} (x${item.quantity}) - ₦${Number(item.unit_price).toLocaleString()}`)
      .join("\n");
    ...
    const customerEmailHtml = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;">
        <div style="background:#4a5d23;padding:30px;text-align:center;">
          <h1 style="color:#ffffff;margin:0;font-size:24px;">BRAINHUB</h1>
          <p style="color:#d4e4a5;margin:8px 0 0;font-size:14px;">Order Confirmation</p>
        </div>
          ...
          <table style="width:100%;border-collapse:collapse;margin:20px 0;">
            ...
            <tfoot>
              <tr>
                ...
                <td style="padding:12px;text-align:right;font-weight:bold;font-size:16px;color:#4a5d23;">₦${Number(totalAmount).toLocaleString()}</td>
              </tr>
            </tfoot>
          </table>
          <div style="background:#f9faf5;border-left:4px solid #4a5d23;padding:16px;margin:20px 0;border-radius:4px;">
    ...
    const adminEmailHtml = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;">
        <div style="background:#4a5d23;padding:30px;text-align:center;">
          <h1 style="color:#ffffff;margin:0;font-size:24px;">BRAINHUB</h1>
          <p style="color:#d4e4a5;margin:8px 0 0;font-size:14px;">New Order Notification</p>
        </div>
        <div style="padding:30px;">
